...
const validateLoginBody = (body: loginRequestBody) => {
  if (!body.login?.trim())
    throw new HttpError(400, "El identificador no puede estar vacío");
  if (!body.password?.trim())
    throw new HttpError(400, "La contraseña no puede estar vacío");
};
...
export const loginUserController = async (req: Request, res: Response) => {
  ...
  if (isEmail) {
    reqBody.login.toLocaleLowerCase();
  }
